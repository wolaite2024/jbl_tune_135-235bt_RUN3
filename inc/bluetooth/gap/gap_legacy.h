 ...
 *           function registered by \ref legacy_register_cb with message type as
 *           \ref PERIODIC_INQUIRY_CANCEL_RSP.
 *
 * \ingroup  GAP_BREDR
 */
typedef struct
...
typedef struct
{
    ...
    uint8_t     bd_addr[6];     /**< Bluetooth address of remote device */
    uint8_t    *name;           /**< Buffer of remote device's name */
 ...
 *           registered by \ref legacy_register_cb with message type as \ref VENDOR_EVT.
 *
 * \ingroup  GAP_BREDR
 */
typedef struct
...
typedef enum
{
    ...
    PERIODIC_INQUIRY_RSP        = (LEGACY_MSG_GROUP_INQUIRY | 0x04),    /**< Response of start periodic inquiry. Message data is \ref T_GAP_PERIODIC_INQUIRY_RSP. */
    PERIODIC_INQUIRY_CANCEL_RSP = (LEGACY_MSG_GROUP_INQUIRY | 0x05),    /**< Response of stop periodic inquiry. Message data is \ref T_GAP_PERIODIC_INQUIRY_CANCEL_RSP. */
    ...
    SETUP_QOS_CMPL       = (LEGACY_MSG_GROUP_ACL | 0x0E),   /**< Result of setup quality of service. Message data is \ref T_GAP_SETUP_QOS_CMPL. */
    ...
    SET_TX_POWER_RSP           = (LEGACY_MSG_GROUP_VND | 0x02), /**< Response of set link tx power. Message data is \ref T_GAP_SET_TX_POWER_RSP. */
    SET_RSSI_GOLDEN_RANGE_RSP  = (LEGACY_MSG_GROUP_VND | 0x03), /**< Response of set rssi golden range. Message data is \ref T_GAP_SET_RSSI_GOLDEN_RANGE_RSP. */
} T_LEGACY_MSG;
 ...
 *           sent, multiple messages will be received in GAP legacy callback function
 *           registed by \ref legacy_register_cb. The first one is \ref PERIODIC_INQUIRY_RSP
 *           which indicates whether inquiry was started successfully. If inquiry
 *           was started successfully, one or more \ref INQUIRY_RESULT may be received,
 *           each message represents a remote device infomation.
 *           After inquiry procedure is done, \ref INQUIRY_CMPL will be received.
 ...
 * <b>Example usage</b>
   ...
   void legacy_cb(void *buf, T_LEGACY_MSG legacy_msg)
   {
       ...
       case PROTO_REG_CMPL:
           {
               legacy_start_periodic_inquiry(false, 8);
           }
           break;

       case PERIODIC_INQUIRY_RSP:
           {
       ...
       case INQUIRY_RESULT:
           {
               T_GAP_INQUIRY_RESULT_INFO *p_info = (T_GAP_INQUIRY_RESULT_INFO *)buf;
               APP_PRINT_TRACE2("Periodic inqury result: remote addr %s, name %s",
                                TRACE_BDADDR(p_info->bd_addr), TRACE_STRING(p_info->name));
           }
           break;
 ...
 * \ingroup  GAP_BREDR
 */
T_GAP_CAUSE legacy_start_periodic_inquiry(bool limited_inquiry, uint16_t max_period_length,
 ...
 *           \ref PERIODIC_INQUIRY_CANCEL_RSP message will be received in GAP legacy callback
 *           function registed by \ref legacy_register_cb to indicate whether inquiry
 *           was successfully stopped. If inquiry was successfully stopped, no
 *           \ref INQUIRY_CMPL message will be received.
 *
 ...
 * <b>Example usage</b>
   ...
   void legacy_cb(void *buf, T_LEGACY_MSG legacy_msg)
   {
       ...
       case PROTO_REG_CMPL:
           {
               legacy_start_periodic_inquiry(false, 5, 3, 8);
           }
           break;
       ...
       case PERIODIC_INQUIRY_RSP:
           {
               ...
               if (!p_rsp->cause)
               {
                   legacy_stop_periodic_inquiry();
               }
           }
           break;

       case PERIODIC_INQUIRY_CANCEL_RSP:
           {
 ...
 * \ingroup  GAP_BREDR
 */
T_GAP_CAUSE legacy_stop_periodic_inquiry(void);
...
T_GAP_CAUSE legacy_vendor_data_rate_set(uint8_t rate);

T_GAP_CAUSE legacy_vendor_tx_power_set(uint16_t handle, int8_t offset_level);

T_GAP_CAUSE legacy_vendor_rssi_golden_range_set(uint16_t handle, int16_t max_rssi,
                                                int16_t min_rssi);

bool legacy_reg_sdp_record(void *p_buf);

bool legacy_del_sdp_record(void *p_buf);

uint8_t legacy_get_max_sdp_record_num(void);

bool legacy_init_radio_mode(void);

T_GAP_CAUSE legacy_set_sco_retrans_window(uint16_t handle, uint8_t retrans_window);
