...
static uint8_t ota_timer_queue_id = 0;
static void *timer_handle_ota = NULL;
...
void ota_timeout_cb(uint8_t timer_id, uint16_t timer_chann)
{
    APP_PRINT_TRACE2("ota_timeout_cb: timer id 0x%02X, timer channel %d", timer_id,
                     timer_chann);
    switch (timer_id)
    {
    case OTA_TIMER_DELAY_RESET:
        {
            gap_stop_timer(&timer_handle_ota);
            dfu_fw_active_reset();
        }
        break;
    ...
    case CMD_OTA_ACTIVE_RESET:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    erase_ota_header_of_active_bank();
#endif
                    gap_start_timer(&timer_handle_ota, "ota_time",
                                    ota_timer_queue_id, OTA_TIMER_DELAY_RESET, 0,
                                    (1 * 1000));
...
void app_ota_init(void)
{
    ...
    gap_reg_timer_cb(ota_timeout_cb, &ota_timer_queue_id);
}
