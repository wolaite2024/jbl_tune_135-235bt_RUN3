...
extern void dsp_state_machine(uint8_t event, uint32_t parameter);


static uint8_t dut_timer_queue_id = 0;
static void *timer_handle_dut_tone_repeat = NULL;
#if F_APP_DUT_MODE_AUTO_POWER_OFF
static void *timer_handle_dut_mode_auto_power_off = NULL;
#endif
static void dut_timer_callback(uint8_t timer_id, uint16_t timer_chann);


typedef enum
{
    HCI_INIT_STATE_VENDOR_LPS_SETTING,
...
static void dut_hci_cmd_vendor_lps_setting(void)
{
    uint16_t opcode = HCI_VENDOR_LPS_SETTING_OPCODE;
    uint8_t param_len = 3;
...
static void dut_hci_cmd_write_local_name(uint8_t *p_name, uint8_t len)
{
    uint16_t opcode = HCI_WRITE_LOCAL_NAME_OPCODE;
    uint8_t param_len = len;
...
static void dut_test_handle_hci_evt_data_ind(bool status, uint8_t *p_buf, uint32_t len)
{
    ...
    {
    case HCI_INIT_STATE_VENDOR_LPS_SETTING:
        {
            dut_hci_cmd_vendor_lps_setting();
        }
        break;
...
{
    if (timer_handle_dut_mode_auto_power_off == NULL)
    {
        gap_start_timer(&timer_handle_dut_mode_auto_power_off, "dut_mode_auto_power_off",
                        dut_timer_queue_id, APP_IO_TIMER_DUT_MODE_AUTO_POWER_OFF, 0, 300000); // 5 mins
    }
}
#endif
    ...
    case SYS_EVENT_POWER_ON:
        {
            ...
            if (app_cfg_const.enable_repeat_dut_test_tone)
            {
                ...
                gap_start_timer(&timer_handle_dut_tone_repeat, "dut_tone_repeat",
                                dut_timer_queue_id, APP_IO_TIMER_DUT_TONE_REPEAT, 0, 5000);
            }
            else
            {
                app_audio_tone_type_play(TONE_DUT_TEST, false, false);
...
#if F_APP_DUT_MODE_AUTO_POWER_OFF
            gap_stop_timer(&timer_handle_dut_mode_auto_power_off);
#endif
...
static void dut_timer_callback(uint8_t timer_id, uint16_t timer_chann)
{
    ...
    case APP_IO_TIMER_DUT_TONE_REPEAT:
        {
            if (timer_handle_dut_tone_repeat != NULL)
            {
                gap_stop_timer(&timer_handle_dut_tone_repeat);
                app_audio_tone_type_play(TONE_DUT_TEST, false, false);
                gap_start_timer(&timer_handle_dut_tone_repeat, "dut_tone_repeat",
                                dut_timer_queue_id, APP_IO_TIMER_DUT_TONE_REPEAT, 0, 5000);
            }
...
#if F_APP_DUT_MODE_AUTO_POWER_OFF
    case APP_IO_TIMER_DUT_MODE_AUTO_POWER_OFF:
        {
            if (timer_handle_dut_mode_auto_power_off != NULL)
            {
                gap_stop_timer(&timer_handle_dut_mode_auto_power_off);
...
void mp_hci_test_init(T_MP_HCI_TEST_ITEM mp_hci_test_item)
{
    ...
    gap_reg_timer_cb(dut_timer_callback, &dut_timer_queue_id);
