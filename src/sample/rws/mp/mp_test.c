...
extern uint32_t h2d_cmd_send(uint8_t *buffer, uint16_t length, uint8_t flag);
    ...
    else
    {
        if (is_mp_test_mode())
        {
            ...
            case MP_TEST_RTK_SUB_OPCODE_EXIT_TEST_MODE:
                {
                    ...
                    mp_test_send_test_result(app_idx, cmd_path, MP_TEST_RESULT_PASS, MP_TEST_OPCODE_RTK,
                                             sub_opcode, NULL, 0);
                    os_delay(500);
                    legacy_send_acl_disconn_req(app_db.br_link[app_idx].bd_addr);
                }
                break;
            ...
            case MP_TEST_RTK_SUB_OPCODE_AUDIO_TEST:
                {
                    uint32_t ret = h2d_cmd_send(p_param, param_len, 1);
                    if (ret == 1)
                    {
                    ...
                    {
                        APP_PRINT_ERROR1("MP_TEST_RTK_SUB_OPCODE_AUDIO_TEST: h2d_cmd_send fail %d", ret);
                    }
                }
                break;
