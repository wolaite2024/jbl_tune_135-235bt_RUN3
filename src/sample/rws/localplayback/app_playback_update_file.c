...
*****************************************************************************************
   * @file      app_playback_update_file.c
...
T_REMOTE_RELAY_HANDLE app_playback_trans_relay_handle;
T_AUDIO_FS_HANDLE playback_write_fs_handle = NULL;
...
static void app_playback_trans_clear_local(void)
{
    ...
    {
        os_mem_free(pb_trans_struct.p_temp_buf_A_head);
        pb_trans_struct.p_temp_buf_A_head = NULL;
    }
    if (pb_trans_struct.p_temp_buf_B_head != NULL)
    {
        os_mem_free(pb_trans_struct.p_temp_buf_B_head);
    ...
    gap_stop_timer(&pb_trans_struct.timer_handle_pb_up_transfer); // ble use
    gap_stop_timer(&pb_trans_struct.timer_handle_file_trans);

    if (playback_write_fs_handle != NULL)
    {
        uint16_t res = audio_fs_close(playback_write_fs_handle);
        if (res != 0)
        {
...
static void app_playback_trans_file_timeout_cb(uint8_t timer_id, uint16_t timer_chann)
{
    APP_PRINT_TRACE1("app_playback_trans_file_timeout_cb: timer id 0x%02X", timer_id);

    switch (timer_id)
    {
    case PB_TIMER_FILE_TRANS:
        {
            if (pb_trans_struct.timer_handle_file_trans != NULL)
            {
                gap_stop_timer(&pb_trans_struct.timer_handle_file_trans);
...
void app_playback_trans_get_device_info(PLAYBACK_DEVICE_INFO *p_deviceinfo)
{
    ...
    p_deviceinfo->song_format_type = PB_FORMAT_TYPE_AAC;
}
/**
...
//    uint8_t res = PB_TRANS_RET_SUCCESS;
    ...
    if (pb_trans_struct.cur_offset < pb_trans_struct.file_total_length)
    {
        ...
        uint32_t read_len;
        APP_PRINT_TRACE2("filetype:0x%x,send_data_len:0x%x", pb_trans_struct.file_list_type, send_data_len);
...
//            res = PB_TRANS_RET_READ_ERROR;
        ...
        if (pb_trans_struct.cur_offset != pb_trans_struct.file_total_length)
        {
            if (pb_trans_struct.trans_mode == PLAYBACK_TRANS_BLE_MODE)
            {
                gap_start_timer(&pb_trans_struct.timer_handle_pb_up_transfer, "playback_up_transfer",
                                pb_trans_struct.timer_upstream_queue_id, PB_TIMER_BLE_UP_FILE_TRANS, 0, 100);
            }
        }
        else
        {
...
static uint8_t app_playback_trans_get_create_file_handle(uint8_t *p_data)
{
    ...
    playback_write_fs_handle = audio_fs_open(file_name, file_name_len, FA_CREATE_ALWAYS | FA_WRITE);
    if (playback_write_fs_handle == NULL)
    {
...
//    res = PB_TRANS_RET_SUCCESS;
//    APP_PRINT_TRACE1("end mem, %d",mem_peek());
    if (pb_trans_struct.p_temp_buf_A_head != NULL)
    {
        os_mem_free(pb_trans_struct.p_temp_buf_A_head);
        pb_trans_struct.p_temp_buf_A_head = NULL;
...
static uint8_t app_playback_trans_data_packet_handle(uint8_t *p_data, uint16_t length)
{
           ...
           pb_trans_struct.cur_offset) == pb_trans_struct.file_total_length)))
    {
        if (results == PB_TRANS_RET_SUCCESS)
        {
            ...
            app_playback_write_send_msg(PLAYBACK_BUF_A_EVENT); // flush buf A to sd
                ...
                pb_trans_struct.cur_offset) == pb_trans_struct.file_total_length)))
    {
        if (results == PB_TRANS_RET_SUCCESS)
        {
            ...
            app_playback_write_send_msg(PLAYBACK_BUF_B_EVENT); // flush buf B to sd
            pb_trans_struct.buf_pingpang_flag = false;
        }
    }
    return results;
...
uint8_t app_playback_fs_write_data(char event)
{
    uint8_t res = PB_TRANS_RET_SUCCESS;
    ...
    APP_PRINT_TRACE1("app_playback_fs_write_data: write flash, cur_offset: 0x%x",
                     pb_trans_struct.cur_offset);
    if (event == PLAYBACK_BUF_A_EVENT)
    {
        uint32_t wrd_len;
        write_res = audio_fs_write(playback_write_fs_handle,
    ...
    return res;
L_Return_Err:
    APP_PRINT_ERROR2("app_playback_fs_write_data,Fail,write_result:%d, res:0x%x", write_res, res);
    return res;
}
...
//    uint16_t local_file_crc;
    ...
    local_file_len = audio_fs_size(playback_write_fs_handle);
    ...
    if (results == PB_TRANS_RET_SUCCESS)
    {
        results = audio_fs_add_file(playback_write_fs_handle, 0);
        if (results != 0)
        {
            APP_PRINT_ERROR0("app_playback_trans_valid_handle audio_fs_add_file fail");
        }
    }
    results = audio_fs_close(playback_write_fs_handle);
    if (results != 0)
    {
...
static uint8_t app_playback_trans_stop_and_del_file(void)
{
    ...
    {
        uint8_t *file_name;
        uint16_t name_len = audio_fs_get_filenameLen(playback_write_fs_handle);
        uint8_t *delete_file = (uint8_t *)malloc(name_len);
        if (delete_file != NULL)
        {
            file_name = audio_fs_get_filename(playback_write_fs_handle);
        ...
        res = audio_fs_close(playback_write_fs_handle);
        playback_write_fs_handle = NULL;
        if (res != 0)
        {
            res = PB_TRANS_RET_CLOSE_ERROR;
...
static uint8_t app_playback_trans_list_updata_handle(uint16_t event_id, uint8_t *p_data)
{
    ...
    file_name = (uint8_t *)os_mem_alloc(RAM_TYPE_DATA_ON, (trans_info.name_len + 1));
    if (file_name == NULL)
    {
        res = PB_TRANS_RET_OPERATION_ERROR;
        return res;
                                                         ...
                                                         uint8_t *p_data, uint16_t len)
{
    ...
    if (PB_TRANS_RET_SUCCESS == results) //pri success
    {
        ...
        uint8_t *buf = (uint8_t *)os_mem_alloc(RAM_TYPE_DATA_ON, buf_len);
        if (buf == NULL)
        {
            return;
        }
        ...
        remote_async_msg_relay(app_playback_trans_relay_handle, PB_REMOTE_MSG_PLAYLIST_UPDATA_SYNC, buf,
                               buf_len, false);
        os_mem_free(buf);
    }
    //ack host
    ...
    {
        app_playback_service_prepare_send_notify(pb_trans_struct.id.ble_conn_id, event_id, sizeof(results),
                                                 &results);
    }
}
/**
...
static void app_playback_trans_couple_exit_handle(uint16_t event_id)
{
    ...
    if (remote_session_state_get() == REMOTE_SESSION_STATE_CONNECTED)
    {
        ...
        buf[0] = results;
        LE_UINT16_TO_ARRAY((buf + sizeof(results)), event_id);
        remote_async_msg_relay(app_playback_trans_relay_handle, PB_REMOTE_MSG_EXIT_SYNC, buf,
                               buf_len, false);
        os_mem_free(buf);
    }
}
                                                    ...
                                                    uint8_t *p_data, uint16_t len)
{
    ...
    if ((PB_TRANS_RET_SUCCESS == results) || (PB_TRANS_RET_DELETE_OPENED_ERROR == results))
    {
        ...
        {
            remote_async_msg_relay(app_playback_trans_relay_handle, PB_REMOTE_MSG_DELETE_FILE_SYNC,
                                   buf, buf_len, false);
        }
        else if (PB_TRANS_RET_DELETE_OPENED_ERROR == results)
        {
            remote_sync_msg_relay(app_playback_trans_relay_handle, PB_REMOTE_MSG_DELETE_OPENED_FILE_SYNC,
                                  buf, buf_len, REMOTE_TIMER_HIGH_PRECISION, 40, false);
        }
        os_mem_free(buf);
    }
    else if (pb_trans_struct.trans_mode == PLAYBACK_TRANS_SPP_MODE)
...
static void app_playback_trans_couple_delete_all_handle(uint16_t event_id)
{
    ...
    if (PB_TRANS_RET_SUCCESS == results) //pri success
    {
        ...
        buf[0] = results;
        LE_UINT16_TO_ARRAY((buf + sizeof(results)), event_id);
        remote_async_msg_relay(app_playback_trans_relay_handle, PB_REMOTE_MSG_DELETE_ALL_FILE_SYNC, buf,
                               buf_len, false);
        os_mem_free(buf);
    }
    else if (pb_trans_struct.trans_mode == PLAYBACK_TRANS_SPP_MODE)
...
static void app_playback_trans_couple_get_device_info(uint16_t event_id)
{
    ...
    if (remote_session_state_get() == REMOTE_SESSION_STATE_CONNECTED)
    {
        ...
        buf[0] = results;
        LE_UINT16_TO_ARRAY((buf + sizeof(results)), event_id);
        remote_async_msg_relay(app_playback_trans_relay_handle, PB_REMOTE_MSG_START_SYNC, buf,
                               sizeof(buf), false);
    }
    ...
    case CMD_PLAYBACK_TRANS_START:
        {
            ...
            if (length == cmd_len)
            {
                ...
                {
                    gap_stop_timer(&pb_trans_struct.timer_handle_sd_active);
                }
    ...
    case CMD_PLAYBACK_TRANS_CONTINUE:
        {
            ...
            ack_pkt[2] = results;
            gap_stop_timer(&pb_trans_struct.timer_handle_file_trans);
            gap_start_timer(&pb_trans_struct.timer_handle_file_trans, "playback_trans_file",
                            pb_trans_struct.file_timer_queue_id, PB_TIMER_FILE_TRANS, 0, 800);
        }
        break;
...
/**
    * @brief  timeout callback for upstream transfer
    * @param  timer_id  timer id
    * @param  timer_chann  time channel
    * @return void
    */
static void app_playback_trans_up_timeout_cb(uint8_t timer_id, uint16_t timer_chann)
{
    APP_PRINT_TRACE1("app_playback_trans_up_timeout_cb: timer id 0x%02X", timer_id);

    switch (timer_id)
    {
    case PB_TIMER_BLE_UP_FILE_TRANS:
        {
            if (pb_trans_struct.timer_handle_pb_up_transfer != NULL)
            {
                gap_stop_timer(&pb_trans_struct.timer_handle_pb_up_transfer);
...
T_APP_RESULT app_playback_ble_handle_cp_req(uint8_t conn_id, uint16_t length, uint8_t *p_value)
{
...
//    p = p_value + 2;
    ...
    case CMD_PLAYBACK_QUERY_INFO:
        {
            if (length == PLAYBACK_LENGTH_GET_INFO)
            {
                ...
                app_playback_service_prepare_send_notify(conn_id, EVENT_PLAYBACK_QUERY_INFO, sizeof(device_info),
                                                         (uint8_t *)&device_info);
    ...
    case CMD_PLAYBACK_GET_LIST_DATA:
        {
            if (length == PLAYBACK_LENGTH_GET_LIST_DATA)
            {
                ...
                {
                    app_playback_trans_error_clear_local();
                    app_playback_service_prepare_send_notify(conn_id, EVENT_PLAYBACK_GET_LIST_DATA, sizeof(results),
                                                             &results);
                    break;
                }
                app_playback_service_prepare_send_notify(conn_id, EVENT_PLAYBACK_GET_LIST_DATA, sizeof(list_info),
                                                         (uint8_t *)&list_info);
                gap_start_timer(&pb_trans_struct.timer_handle_pb_up_transfer, "playback_up_transfer",
                                pb_trans_struct.timer_upstream_queue_id, PB_TIMER_BLE_UP_FILE_TRANS, 0, 100);
            }
    ...
    case CMD_PLAYBACK_EXIT_TRANS:
        {
            ...
            {
                app_playback_trans_exit_trans_handle();
                app_playback_service_prepare_send_notify(conn_id, EVENT_PLAYBACK_EXIT_TRANS, sizeof(results),
                                                         &results);
            }
            else
            {
    ...
    case CMD_PLAYBACK_PERMANENT_DELETE_SONG:
        {
            ...
            if (length == (rx_file_len + sizeof(PB_DELETE_FILE_STRUCT)))
            {
                ...
                {
                    results = app_playback_trans_single_delete_handle(p);
                    app_playback_service_prepare_send_notify(conn_id, EVENT_PLAYBACK_PERMANENT_DELETE_SONG,
    ...
    case CMD_PLAYBACK_PERMANENT_DELETE_ALL_SONG:
        {
            if (length == PLAYBACK_LRNGTH_DELETE_ALL_SONG)
            {
                ...
                {
                    results = app_playback_trans_delete_all_file();
                    app_playback_service_prepare_send_notify(conn_id, EVENT_PLAYBACK_PERMANENT_DELETE_ALL_SONG,
                                                             sizeof(results), &results);
                }
                else
                {
    ...
    case CMD_PLAYBACK_PLAYLIST_DELETE_SONG:
        {
            ...
            if (length == (rx_file_len + sizeof(PB_PLAYLIST_UPDATE_STRUCT)))
            {
                ...
                {
                    results = app_playback_trans_list_updata_handle(playlist_event_id, p);
                    app_playback_service_prepare_send_notify(conn_id, playlist_event_id, sizeof(results),
                                                             &results);
                }
                else
                {
    ...
    default:
        APP_PRINT_ERROR1("app_playback_ble_handle_cp_req, cmd_id not expected", cmd_id);
        break;
    }

    if (error_flag)
    {
        APP_PRINT_ERROR0("app_playback_ble_handle_cp_req: invalid length");
    }
...
static void app_playback_trans_relay_playlist_sec_updata_handle(uint8_t *buf, uint16_t len)
{
    ...
    remote_async_msg_relay(app_playback_trans_relay_handle, PB_REMOTE_MSG_SEC_RESULT_SYNC, data,
                           sizeof(data), false);
...
static void app_playback_trans_relay_sec_exit_handle(uint8_t *buf, uint16_t len)
{
    ...
    //ack pri
    buf[0] = results;
    remote_async_msg_relay(app_playback_trans_relay_handle, PB_REMOTE_MSG_SEC_RESULT_SYNC, buf,
                           len, false);
}
/**
...
static void app_playback_trans_relay_sec_result_handle(uint8_t *buf, uint16_t len)
{
    ...
    {
        app_playback_service_prepare_send_notify(pb_trans_struct.id.ble_conn_id, event_id,
                                                 sizeof(results), &results);
    }

}

void app_playback_trans_relay_cback(uint16_t event, T_REMOTE_RELAY_STATUS status, void *buf,
                                    uint16_t len)
{
    APP_PRINT_TRACE4("app_playback_trans_relay_cback: event 0x%04x, status %d, len:%d, bud_role: %d",
    ...
    case PB_REMOTE_MSG_SEC_RESULT_SYNC:
        ...
        break;
    case PB_REMOTE_MSG_SEC_INIT_RESULT_SYNC:
        break;
    case PB_REMOTE_MSG_RESUME_SYNC:
        break;
...
/////////////////////////////////////////sd active start/////////////////////////////////////////////////////
/**
    * @brief  timeout callback for sd active
    * @param  timer_id  timer id
    * @param  timer_chann  time channel
    * @return void
    */
static void app_playback_trans_sd_active_timeout_cb(uint8_t timer_id, uint16_t timer_chann)
{
    APP_PRINT_TRACE1("app_playback_trans_sd_active_timeout_cb: timer id 0x%02X", timer_id);

    switch (timer_id)
    {
    case PLAYBACK_SD_ACTIVE_TIME_OUT:
        {
            if (pb_trans_struct.timer_handle_sd_active != NULL)
            {
                gap_stop_timer(&pb_trans_struct.timer_handle_sd_active);
...
void app_playback_trans_sd_active_start_timer(uint8_t time_s)
{
    gap_stop_timer(&pb_trans_struct.timer_handle_sd_active);
    ...
    gap_start_timer(&pb_trans_struct.timer_handle_sd_active, "app_playback_trans_sd_active_start_timer",
                    pb_trans_struct.sd_active_timer_queue_id, PLAYBACK_SD_ACTIVE_TIME_OUT, 0, time_s * 1000);
}
...
{
    gap_reg_timer_cb(app_playback_trans_up_timeout_cb, &pb_trans_struct.timer_upstream_queue_id);
    gap_reg_timer_cb(app_playback_trans_file_timeout_cb, &pb_trans_struct.file_timer_queue_id);
    gap_reg_timer_cb(app_playback_trans_sd_active_timeout_cb,
                     &pb_trans_struct.sd_active_timer_queue_id);
#if APP_PLAYBACK_TRANS_COUPLE_EN
    app_playback_trans_relay_handle = remote_relay_register(app_playback_trans_relay_cback);
#endif
    create_playback_write_task();
}
...
static void app_playback_trans_write_result_ack(uint8_t wr_result)
{
    ...
    {
        APP_PRINT_ERROR0("app_playback_trans_write_result_ack: msg send fail");
    }
}
...
void app_playback_write_send_msg(char event)
{
    os_msg_send(playback_wr_msg_queue, &event, 0);
    APP_PRINT_TRACE1("app_playback_write_send_msg,event:%d", event);
}

static void app_playback_write_data_to_sd(char event)
{
    uint8_t results = PB_TRANS_RET_SUCCESS;

    results = app_playback_fs_write_data(event);
//    app_report_event(CMD_PATH_SPP, EVENT_PLAYBACK_REPORT_BUFFER_CHECK, pb_trans_struct.id.spp_idx,
//                     &results, sizeof(results));
//    send app msg
...
static void playback_write_data_task(void *arg)
{
    char event;

    os_msg_queue_create(&playback_wr_msg_queue, "playback_msgQ", MAX_QUEUE_SIZE_PLAYBACK, sizeof(char));
    while (true)
    {
        if (os_msg_recv(playback_wr_msg_queue, &event, 0xFFFFFFFF) == true)
        {
            ...
            {
                app_playback_write_data_to_sd(event);
            }
        }
    }
}

void create_playback_write_task(void)
{
    if (os_task_create(&playback_write_task_handle, "playback_write_data", playback_write_data_task,
                       NULL, 1024, 1) == true)
    {
        APP_PRINT_TRACE0("create_playback_write_task success");
    }
}
