    ...
    case BT_EVENT_AVRCP_ELEM_ATTR:
        {
            ...
            if (p_link != NULL)
            {
                ...
                {
                    remote_async_msg_relay(listen_save_db.relay_handle, LISTEN_SAVE_REMOTE_MSG_FILE_INFO_SYNC,
                                           (uint8_t *)p_listen_save_file, file_info_len, false);
                }
            }
        }
        break;
...
//                                 param->avrcp_get_play_status_rsp.position_ms);
    ...
    case BT_EVENT_A2DP_SNIFFING_CONN_CMPL:
        {
            p_link = app_find_br_link(param->a2dp_sniffing_conn_cmpl.bd_addr);
            if (p_link != NULL)
            {
    ...
    case BT_EVENT_A2DP_STREAM_DATA_IND:
        {
                     ...
                     (listen_saveing_state == LISTEN_SAVE_DOING))
            {
                ...
                if (p_sync_pkt->end_flag == 1)
                {
                    ...
                    gap_start_timer(&listen_save_db.timer_handle, "listen and save end",
                                    listen_save_db.timer_queue_id,
                                    LISTEN_SAVE_TIMER_SAVE_END, 0, 30);
                }
            }
...
void app_listen_save_mode_enter(void)
{
    ...
    if (remote_session_state_get() == REMOTE_SESSION_STATE_CONNECTED)
    {
        ...
        {
            uint8_t buf = 0;
            remote_async_msg_relay(listen_save_db.relay_handle, LISTEN_SAVE_REMOTE_MSG_CREATE_FILE_SYNC,
                                   (uint8_t *)&buf, sizeof(buf), false);
        }
    }
    else if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_SINGLE)
    {
...
static void app_listen_save_pri_sync_pkt_info(void *buf)
{
    ...
    {
        remote_async_msg_relay(listen_save_db.relay_handle,
                               LISTEN_SAVE_REMOTE_MSG_FRAME_INFO_SYNC,
                               p_sync_pkt,
                               sizeof(T_LISTEN_SAVE_PRI_PKT_INFO),
                               false);
        ...
        if (p_sync_pkt->end_flag == 1)
        {
            ...
            gap_start_timer(&listen_save_db.timer_handle, "listen and save end",
                            listen_save_db.timer_queue_id,
                            LISTEN_SAVE_TIMER_SAVE_END, 0, 60);
        }
    }
}
...
static uint8_t app_listen_save_sec_sync_pkt_info(void *buf)
{
    ...
    if ((p_wait_pkt->pkt_num != 0) && (listen_save_cacu_diff_u16(x_seq, y_seq) < 0))
    {
        do
        {
        ...
        while (p_wait_pkt->pkt_num && (listen_save_cacu_diff_u16(x_seq, y_seq) < 0));
    }
...
static void app_listen_save_relay_sec_create_result_handle(uint8_t *buf, uint16_t len)
{
    ...
    if (*((uint8_t *)buf + 0) == LISTEN_SAVE_SUCCESS)
    {
        ...
        if ((listen_save_db.pri_flag == true) && (listen_save_db.sec_flag == true))
        {
            ...
            uint8_t buf = 0;
            remote_sync_msg_relay(listen_save_db.relay_handle, LISTEN_SAVE_REMOTE_MSG_SAVING_SYNC,
                                  (uint8_t *)&buf, sizeof(buf), REMOTE_TIMER_HIGH_PRECISION, 40, false);
            listen_save_db.pri_flag = false;
            listen_save_db.sec_flag = false;
        }
    }
...
static uint8_t app_listen_save_sec_one_pkt_check_save(T_LISTEN_SAVE_SYNC_INFO pri_pkt)
{
    ...
    if ((p_peek_pkt != NULL) && (listen_save_cacu_diff_u16(p_peek_pkt->seq, pri_pkt.seq_num) > 0))
    {
...
static int listen_save_cacu_diff_u16(uint16_t x, uint16_t y)
{
    int seq_diff;
    if (x > y)
    {
...
void app_listen_save_relay_cback(uint16_t event, T_REMOTE_RELAY_STATUS status, void *buf,
                                 uint16_t len)
{
    uint8_t res;
    APP_PRINT_TRACE4("app_listen_save_relay_cback: event 0x%04x, status %d, len:%d, bud_role:%d",
    ...
    case LISTEN_SAVE_REMOTE_MSG_CREATE_FILE_SYNC:
            ...
            status == REMOTE_RELAY_STATUS_ASYNC_SENT_OUT)
        {
            ...
            if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_SECONDARY)
            {
                remote_async_msg_relay(listen_save_db.relay_handle, LISTEN_SAVE_REMOTE_MSG_SEC_CREATE_RESULT_SYNC,
                                       (uint8_t *)&res, sizeof(res), false);
            }
            else if ((app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY) && (res == LISTEN_SAVE_SUCCESS))
            {
                ...
                if ((listen_save_db.pri_flag == true) && (listen_save_db.sec_flag == true))
                {
                    ...
                    uint8_t buf = 0;
                    remote_sync_msg_relay(listen_save_db.relay_handle, LISTEN_SAVE_REMOTE_MSG_SAVING_SYNC,
                                          (uint8_t *)&buf, sizeof(buf), REMOTE_TIMER_HIGH_PRECISION, 60, false);
                    listen_save_db.pri_flag = false;
                    listen_save_db.sec_flag = false;
                }
            }
...
static void app_listen_save_timer_save_end_handle(void)
{
    ...
    if (remote_session_state_get() == REMOTE_SESSION_STATE_CONNECTED)
    {
        ...
        {
            uint8_t buf = 0;
            remote_sync_msg_relay(listen_save_db.relay_handle, LISTEN_SAVE_REMOTE_MSG_SAVE_END_SYNC,
                                  (uint8_t *)&buf, sizeof(buf), REMOTE_TIMER_HIGH_PRECISION, 40, false);
        }
    }
    else if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_SINGLE)
...
static void listen_save_timer_callback(uint8_t timer_id, uint16_t timer_chann)
{
    APP_PRINT_TRACE2("listen_save_timer_callback: timer_id 0x%02x, timer_chann 0x%x",
                     timer_id, timer_chann);

    switch (timer_id)
    {
    case LISTEN_SAVE_TIMER_SAVE_END:
        {
            gap_stop_timer(&listen_save_db.timer_handle);
...
void app_listen_save_init(void)
{
    ...
    listen_save_db.pri_flag = false;
    listen_save_db.sec_flag = false;
    listen_save_db.relay_handle = remote_relay_register(app_listen_save_relay_cback);
    ...
    memset((uint8_t *)p_wait_pkt, 0, sizeof(T_LISTEN_SAVE_WAIT_PKT_INFO));
    gap_reg_timer_cb(listen_save_timer_callback, &listen_save_db.timer_queue_id);
}
#endif
