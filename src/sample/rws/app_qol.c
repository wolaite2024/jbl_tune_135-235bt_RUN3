...
/*for qol*/
#define APP_ROLESWAP_TIMER_JUDGE_INTERVAL  0
#define APP_BUFFER_LEVEL_REPORT            1
#define APP_START_SNIFFING_CHECK_INTERVAL  2
...
static uint8_t link_monitor_timer_queue_id = 0;
static void *timer_handle_link_monitor = NULL;
static void *timer_handle_buffer_level_report = NULL;
static void *timer_handle_b2b_link_monitor = NULL;
...
static int8_t app_qol_dequeue(T_ALIST *qlist)
{
    ...
    // AUDIO_PRINT_TRACE4("app_qol_dequeue:%d, rear:%d, size:%d data:%d", qlist->front,
...
static void app_qol_sliding_interval_hdlr(void)
{
    ...
    if (rssi_average + app_cfg_const.roleswap_rssi_threshold < rssi_average_remote)
    {
        ...
        gap_stop_timer(&timer_handle_link_monitor);
...
static void app_qol_link_monitor_timer_callback(uint8_t timer_id, uint16_t timer_chann)
{
    APP_PRINT_TRACE2("app_qol_link_monitor_timer_callback: timer_id 0x%02x, timer_chann %d",
                     timer_id, timer_chann);
    switch (timer_id)
    {
    case APP_ROLESWAP_TIMER_JUDGE_INTERVAL:
        {
            ...
            gap_start_timer(&timer_handle_link_monitor,
                            "link_monitor_update_interval",
                            link_monitor_timer_queue_id,
                            APP_ROLESWAP_TIMER_JUDGE_INTERVAL,
                            1,
                            qol_data.interval);
    ...
    case APP_BUFFER_LEVEL_REPORT:
        {
            ...
            gap_start_timer(&timer_handle_buffer_level_report,
                            "buffer_level_report_interval",
                            link_monitor_timer_queue_id,
                            APP_BUFFER_LEVEL_REPORT,
                            1,
    ...
    case APP_START_SNIFFING_CHECK_INTERVAL:
        {
            gap_stop_timer(&timer_handle_b2b_link_monitor);
            gap_start_timer(&timer_handle_b2b_link_monitor,
                            "b2b_link_monitor_update_interval",
                            link_monitor_timer_queue_id,
                            APP_START_SNIFFING_CHECK_INTERVAL,
                            1,
...
static void app_qol_event_cback(T_BT_EVENT event_type, void *event_buf, uint16_t buf_len)
{
    ...
    case BT_EVENT_LINK_RSSI_INFO:
        {
            if (app_check_b2b_link(param->link_rssi_info.bd_addr))
            {
    ...
    else
    {
        ...
        if (enable)
        {
            ...
            else if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
            {
                ...
                gap_stop_timer(&timer_handle_buffer_level_report);
                gap_start_timer(&timer_handle_buffer_level_report,
                                "buffer_level_report_interval",
                                link_monitor_timer_queue_id,
                                APP_BUFFER_LEVEL_REPORT,
...
void app_qol_init(void)
{
    ...
    gap_reg_timer_cb(app_qol_link_monitor_timer_callback, &link_monitor_timer_queue_id);
