...
#define WaitRdyLow()                    {uint16_t count = 0; while(GPIO_ReadInputDataBit(GPIO_GetPin(app_cfg_const.gsensor_int_pinmux)) && count++ < 20000){;};APP_PRINT_TRACE1("WaitRdyLow iqs %d",count);}

#define WaitRdyHigh()                   {uint16_t count = 0; while(!GPIO_ReadInputDataBit(GPIO_GetPin(app_cfg_const.gsensor_int_pinmux)) && count++ < 10000){;};APP_PRINT_TRACE1("WaitRdyHigh iqs %d",count);}
