    ...
    case REPORT_ID_USB_COMMAND_SET:
        {
            ...
            app_hid_handle_config_report(REPORT_ID_USB_COMMAND_SET, APP_TEAMS_HID_REPORT_SOURCE_USB,
                                         &hid_data);
#endif
        }
        break;
...
uint8_t hid_handle_get_report(uint8_t reportID, uint8_t *data, uint16_t *length)
{
    ...
    {
        result = 0;
        APP_PRINT_ERROR0("hid_handle_get_report: msg send fail");
    }
    return result;
}
...
void hid_handle_get_report_msg(uint8_t reportID, uint16_t get_len)
{
    ...
    case REPORT_ID_USB_COMMAND_GET:
#if F_APP_TEAMS_FEATURE_SUPPORT
        usb_command_get_report_buffer = app_teams_get_usb_response();
        ...
        memset(usb_command_get_report_buffer, 0, APP_COMMAND_MAX_FOR_USB);
#endif
        break;
...
void hid_handle_device_suspend(void)
{
    ...
    if (app_io_send_msg(&gpio_msg) == false)
    {
        APP_PRINT_ERROR0("hid_handle_device_suspend: msg send fail");
    }
}
...
static void app_usb_teams_check_and_notify_call_status(void)
{
    ...
    if (s_last_teams_status != teams_call_status)
    {
        ...
        app_teams_multilink_voice_priority_rearrangment(multilink_usb_idx,
                                                        (T_BT_HFP_CALL_STATUS)teams_call_status);
#endif
    }
