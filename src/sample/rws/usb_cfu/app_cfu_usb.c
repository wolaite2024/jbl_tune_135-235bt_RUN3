...
void app_usb_hid_handle_debug_data(uint8_t *data, uint8_t length);
#endif
...
void app_usb_hid_send_report(uint8_t channel, uint8_t report_id, uint8_t *data, uint8_t length)
{
    int i;
    if (channel == 0)
    {
...
#if hid_debug
#include "gap_timer.h"
uint8_t app_usb_hid_timer_queue_id;
void *app_usb_hid_timer_handle;
void app_usb_hid_timeout_cb(uint8_t timer_id, uint16_t timer_chann)
{
    USB_PRINT_TRACE1("app_usb_hid_timeout_cb channel %x", timer_chann);
    gap_stop_timer(&app_usb_hid_timer_handle);
    uint8_t mouse_data[4] = {0, 11, 11};
    app_usb_hid_send_report(1, 0, mouse_data, 4);
    uint8_t keyboard_data[12] = {0, 0x0a};
    app_usb_hid_send_report(0, 0, keyboard_data, 10);
    void os_delay(uint32_t ms);
    os_delay(10);
    keyboard_data[1] = 0;
    app_usb_hid_send_report(0, 0, keyboard_data, 11);
    gap_start_timer(&app_usb_hid_timer_handle, "app_hid_timer_test",
                    app_usb_hid_timer_queue_id, CFU_TIMER_RESET_ACTIVE, timer_chann + 1, 1000);
}
#endif

#if F_APP_TEAMS_FEATURE_SUPPORT
void app_teams_cfu_handle_set_report(uint8_t *data, uint16_t length)
{
    ...
    case REPORT_ID_CFU_OFFER_OUTPUT:
        {
            ...
            app_teams_cfu_save_report_info(APP_TEAMS_HID_REPORT_SOURCE_USB, APP_TEAMS_HID_REPORT_ACTION_NONE,
                                           0);
...
void addin_hid_handle_set_report(uint8_t *data, uint16_t length)
{
    switch (data[0])
    {
    ...
    case REPORT_ID_CFU_PAYLOAD_OUTPUT:
#if F_APP_TEAMS_FEATURE_SUPPORT
        app_teams_cfu_handle_set_report(data, length);
#else
    ...
    case REPORT_ID_HID_DEBUG_INTERFACE_OUT:
        app_usb_hid_handle_debug_data(data, length);
        break;
#endif
    default:
...
bool addin_hid_handle_get_report(uint8_t reportID, uint8_t *data, uint16_t *length)
{

#if hid_debug
    gap_reg_timer_cb(app_usb_hid_timeout_cb, &app_usb_hid_timer_queue_id);
    gap_start_timer(&app_usb_hid_timer_handle, "app_hid_timer_test",
                    app_usb_hid_timer_queue_id, CFU_TIMER_RESET_ACTIVE, 0, 5000);
#endif
    switch (reportID)
    {
...
void app_usb_hid_send_debug_data(uint8_t type, uint8_t *data, uint8_t length)
{
    int i;
    ...
    USB_PRINT_TRACE1("app_usb_hid_send_debug_data %b",
                     TRACE_BINARY(app_usb_hid_interrupt_in_buff[2] + 2, app_usb_hid_interrupt_in_buff));
}

void app_usb_hid_handle_debug_data(uint8_t *data, uint8_t length)
{
    USB_PRINT_TRACE1("app_usb_hid_handle_debug_data %b", TRACE_BINARY(length, data));
    switch (data[1])
    {
    case 1:
        {
            app_usb_hid_send_debug_data(data[2], &data[4], data[3]);
        }
    }
