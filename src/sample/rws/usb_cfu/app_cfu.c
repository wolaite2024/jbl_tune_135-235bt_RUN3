...
uint8_t app_cfu_timer_queue_id;
void *app_cfu_content_timer_handle;
void *app_cfu_reset_timer_handle;
//////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////
...
void app_cfu_offer_parse(uint8_t *data, uint16_t length)
{
    ...
    if (offerCommand->last_img_id < IMG_OCCD || offerCommand->last_img_id > IMG_MAX)
    {
         ...
         m_cfu_struct.offer_info[i].ComponentInfo.ComponentId != 0; i++)
    {
        if (m_cfu_struct.offer_info[i].ComponentInfo.ComponentId == offerCommand->ComponentInfo.ComponentId)
        {
            ...
            gap_start_timer(&app_cfu_content_timer_handle, "app_cfu_timer_trans",
                            app_cfu_timer_queue_id, CFU_TIMER_CONTENT_TRANS, 0, 3000);
...
void app_cfu_content_parse(uint8_t *data, uint16_t length)
{
        ...
        COMPONENT_FIRMWARE_UPDATE_FLAG_BUDS_SUCCESS)// two buds  success
    {
        ...
        if (dfu_result == true)// 1 means dfu checksum pass
        {
            ...
            {
                if (app_cfu_content_timer_handle != NULL)
                {
                    gap_stop_timer(&app_cfu_content_timer_handle);
                }
                    ...
                    (app_cfu_get_version(CFU_VERSION_SECTION, true) >= app_cfu_get_version(CFU_VERSION_SECTION, false)))
                {
                    if (is_ota_support_bank_switch())
                    {
                        ...
                        dfu_erase_img_flash_area(header_addr, 0x20, true);
                    }
                }
                m_cfu_struct.update_in_progress = false;
                m_cfu_struct.is_bank_swap_pending = true;
                gap_start_timer(&app_cfu_reset_timer_handle, "app_cfu_timer_reset",
                                app_cfu_timer_queue_id, CFU_TIMER_RESET_ACTIVE, 0, 3000);
...
//                }
                ...
                else  // last_update_img_id is get
                {
                    ...
                    else
                    {
                        ...
                        gap_start_timer(&app_cfu_content_timer_handle, "app_cfu_timer_trans",
                                        app_cfu_timer_queue_id, CFU_TIMER_CONTENT_TRANS, 0, 30000);
                    }
                    contentResponse->HidCfuContentResponse.CfuContentResponse.Status =
                        COMPONENT_FIRMWARE_UPDATE_SINGEL_BUD_SUCCESS;
                }
...
//            app_cfu_content_parse(app_cfu_receive_buffer, m_cfu_struct.usb_receive_length);
    ...
    addin_hid_interrupt_in(app_cfu_response_buffer, sizeof(FWUPDATE_OFFER_RESPONSE));
    return;
}
...
void app_cfu_timeout_cb(uint8_t timer_id, uint16_t timer_chann)
{
    DFU_PRINT_TRACE2("app_cfu_timeout_cb: timer id 0x%02X ch 0x%x", timer_id, timer_chann);
    ...
    case CFU_TIMER_RESET_ACTIVE:
        {
            if (app_cfu_reset_timer_handle == NULL)
            {
                break;
            }
            gap_stop_timer(&app_cfu_reset_timer_handle);
            if (m_cfu_struct.current_updating_compomentID == CFU_SELF_COMPONENT_ID)
            {
                ...
                else
                {
                    ...
                    gap_start_timer(&app_cfu_reset_timer_handle, "app_cfu_timer_reset",
                                    app_cfu_timer_queue_id, CFU_TIMER_RESET_ACTIVE, 0, 10000);
                }
            }
        }
    }
...
void app_cfu_init()
{
    ...
    gap_reg_timer_cb(app_cfu_timeout_cb, &app_cfu_timer_queue_id);
...
//            app_cfu_set_cfu_version(*(uint32_t *)(&data[1]));
    ...
    case 4:
        {
            ...
            uint32_t dfu_result = dfu_update(img_id, offset, length, (test_data + 7));
    ...
    case 5:
        {
            uint16_t img_id = *(uint16_t *)(test_data);
            uint32_t dfu_result = dfu_check_checksum(img_id, 0);
    ...
    case 8:
        {
            ...
            addin_hid_interrupt_in((test_data + 1), length);
        }
        break;
    case 9:
        {
            uint32_t header_addr = get_active_ota_bank_addr();
            dfu_erase_img_flash_area(header_addr, BANK0_OTA_HDR_SIZE, true);
        }
    case 10:
        {
            dfu_fw_active_reset();
