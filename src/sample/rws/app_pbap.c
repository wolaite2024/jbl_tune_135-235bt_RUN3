...
static uint8_t app_pbap_timer_queue_id = 0xFF;
...
static void *timer_handle_pbap_pull_continue = NULL;
...
void app_pbap_pull_continue_timer_start(void)
{
    ...
    gap_start_timer(&timer_handle_pbap_pull_continue, "pbap_pull_continue", app_pbap_timer_queue_id,
                    APP_PBAP_TIMER_PBAP_PULL_CONTINUE, 0, PBAP_PULL_CONTINUE_TIMER);
}
#endif
...
static void app_pbap_timeout_cb(uint8_t timer_id, uint16_t timer_chann)
{
    switch (timer_id)
    {
#if F_APP_PBAP_CMD_SUPPORT
    case APP_PBAP_TIMER_PBAP_PULL_CONTINUE:
        {
            if (timer_handle_pbap_pull_continue != NULL)
            {
                uint8_t app_index = app_hfp_get_active_hf_index();

                gap_stop_timer(&timer_handle_pbap_pull_continue);
    ...
    case CMD_PBAP_DOWNLOAD_CONTROL:
        {
            ...
            case PBAP_DOWNLOAD_CONTROL_ABORT:
                {
                    if (bt_pbap_pull_abort(app_db.br_link[app_index].bd_addr) == false)
                    {
                        ack_pkt[2] = CMD_SET_STATUS_PROCESS_FAIL;
                    }
                    else
            ...
            case PBAP_DOWNLOAD_CONTROL_CONTINUE:
                {
                    if (bt_pbap_pull_continue(app_db.br_link[app_index].bd_addr))
                    {
                        enable_auto_pbap_download_continue_flag = true;
                    }
                    else
...
void app_pbap_init(void)
{
    if (app_cfg_const.supported_profile_mask & PBAP_PROFILE_MASK)
    {
        ...
        gap_reg_timer_cb(app_pbap_timeout_cb, &app_pbap_timer_queue_id);
    }
}
#endif
