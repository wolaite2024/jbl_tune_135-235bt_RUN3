...
static uint8_t anc_queue_id = 0;
static void *timer_handle_measure_kill_acl = NULL;
static void *timer_handle_anc_tool_wait_sec_respond = NULL;
...
typedef enum
{
    APP_IO_TIMER_ANC_MEASURE_KILL_ACL,
    APP_IO_TIMER_ANC_TOOL_WAIT_SEC_RESPOND,
} T_APP_ANC_TIMER;
...
static bool app_anc_ramp_tool_data_initial(uint16_t data_length)
{
    bool ret = false;
...
{
    if (!anc_ramp_tool_data.ramp_burn_start)
    {
...
static bool app_anc_tool_media_buffer_idle(void)
{
    ...
    {
        state = app_teams_multilink_get_usb_ds_track_state();
    }
    else
#endif
    {
    ...
    case EVENT_REPORT_STATUS:
        {
            ...
            report_data[0] = GET_STATUS_ANC_STATUS;
    ...
    case EVENT_ANC_QUERY:
        {
            ...
            report_data[1] = app_anc_is_anc_on_state((T_ANC_APT_STATE)app_db.current_listening_state);
            ...
            app_report_event_broadcast(EVENT_ANC_QUERY, report_data, report_data[3] + 4);
        }
        break;

#if (NEW_FORMAT_LISTENING_CMD_REPORT == 0)
    case EVENT_ANC_ENABLE_DISABLE:
        {
            uint8_t report_data[2];
            uint8_t cmd_status = event_data[0];

            report_data[0] = app_anc_is_anc_on_state((T_ANC_APT_STATE)app_db.current_listening_state);
            report_data[1] = cmd_status;

            app_report_event_broadcast(EVENT_ANC_ENABLE_DISABLE, &report_data[0], sizeof(report_data));
        }
        break;
#endif
    ...
    case CMD_ANC_TEST_MODE:
        {
            ...
            if ((app_bt_policy_get_b2b_connected()) && (path != CMD_PATH_RWS_ASYNC) &&
                (mp_type == MP_TYPE_TWO_DEVICE))
            {
                ...
                {
                    gap_start_timer(&timer_handle_anc_tool_wait_sec_respond, "anc_tool_wait_sec_respond", anc_queue_id,
                                    APP_IO_TIMER_ANC_TOOL_WAIT_SEC_RESPOND, 0, 3000);
                }
                else
                {
            ...
            else if ((app_bt_policy_get_b2b_connected()) && (path == CMD_PATH_RWS_ASYNC))
            {
    ...
    case CMD_ANC_READ_GAIN:
        {
            ...
            */
            ...
            anc_mp_tool_data.report_value[24] = app_anc_is_anc_on_state((T_ANC_APT_STATE)
    ...
    case CMD_ANC_ENABLE_DISABLE:
        {
...
#if (NEW_FORMAT_LISTENING_CMD_REPORT == 0)
            uint8_t is_phone_tool = (param_len > 1);
            uint8_t cmd_ptr[3] = {0};
#endif
            if (enable == ENABLE)
            {
                ...
                if (is_phone_tool)
                {
                    ...
                    else if (param_ptr[1] < app_anc_get_selected_scenario_cnt())
                    {
                        // example:
                        // activate count is 5, parameter 0~4 will be valid
                        cmd_ptr[0] = ANC_STATE_TO_EVENT(ANC_ON_SCENARIO_1_APT_OFF + param_ptr[1]);
                        cmd_ptr[1] = true;
                        cmd_ptr[2] = true;
                    ...
                    {
                        anc_status = CMD_SET_STATUS_PARAMETER_ERROR;
                    }
                }
                else
#endif
            ...
            if (is_phone_tool)
            {
                ...
                app_anc_report(EVENT_ANC_ENABLE_DISABLE, &anc_status, 1);
            }
            else
#endif
            {
    ...
    case CMD_ANC_LLAPT_FEATURE_CONTROL:
        {
            ...
            if ((app_bt_policy_get_b2b_connected()) && (path != CMD_PATH_RWS_ASYNC) &&
                (mp_type == MP_TYPE_TWO_DEVICE))
            {
                uint8_t *event_buff = (uint8_t *)malloc(len + MP_LEN_SUBOPCODE + 6);
    ...
    case CMD_RAMP_GET_INFO:
        {
            ...
            report_data[3] = MAX_RAMP_TOOL_CMD_LENGTH;
            report_data[4] = MAX_RAMP_TOOL_CMD_LENGTH >> 8;
            ...
            APP_PRINT_TRACE5("audio_app_anc ramp_get_info is_engage = %x, channel = %x, supported_size = %d, pri_vol = %d, sec_vol = %d",
                             report_data[1], report_data[2], MAX_RAMP_TOOL_CMD_LENGTH, app_db.local_batt_level,
    ...
    case CMD_RAMP_BURN_PARA_START:
        {
            ...
            bool length_valid = (app_db.remote_session_state == REMOTE_SESSION_STATE_CONNECTED &&
                                ...
                                : (MAX_RAMP_TOOL_CMD_LENGTH >= param_len) ? true : false;
            ...
            {
                if (app_anc_ramp_tool_data_initial(total_length))
                {
                    anc_status = ANC_CMD_STATUS_SUCCESS;
                }
            }
    ...
    case CMD_RAMP_BURN_PARA_CONTINUE:
        {
            ...
            if (channel_match && length_valid)
            {
                ...
                if (anc_status == ANC_CMD_STATUS_SUCCESS)
                {
                    memcpy(&anc_ramp_tool_data.buffer_ptr[anc_ramp_tool_data.data_receive_length], &param_ptr[7],
                           payload_length);
    ...
    case CMD_LISTENING_MODE_CYCLE_SET:
        {
            ...
            if (cycle_mode < 4)
            {
                app_cfg_nv.listening_mode_cycle = cycle_mode;
                report_value = app_cfg_nv.listening_mode_cycle;
...
#if (NEW_FORMAT_LISTENING_CMD_REPORT == 0)
static void app_anc_report_status_change(uint8_t status_on)
{
    if (app_db.remote_session_state == REMOTE_SESSION_STATE_CONNECTED)
    {
        if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
        {
            ...
            {
                app_anc_report(EVENT_REPORT_STATUS, &status_on, 1);
            }
        }

        if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_SECONDARY)
        {
            app_relay_async_single_with_raw_msg(APP_MODULE_TYPE_ANC, APP_REMOTE_MSG_SECONDARY_ANC_STATUS,
                                                &status_on, 1);
        }
    }
    else
    ...
    case AUDIO_EVENT_ANC_ENABLED:
        {
...
#if (NEW_FORMAT_LISTENING_CMD_REPORT == 0)
            app_anc_report_status_change(ENABLE);
#endif
            ...
            APP_PRINT_TRACE1("wade AUDIO_EVENT_ANC_ENABLED stable = %x", anc_status_stable);
    ...
    case AUDIO_EVENT_ANC_DISABLED:
        {
...
#if (NEW_FORMAT_LISTENING_CMD_REPORT == 0)
            app_anc_report_status_change(DISABLE);
#endif
            ...
            APP_PRINT_TRACE1("wade AUDIO_EVENT_ANC_DISABLED stable = %x", anc_status_stable);
...
void app_anc_handle_anc_tool_cmd(uint8_t *cmd_ptr)
{
    ...
    APP_PRINT_TRACE3("app_anc_handle_anc_tool_cmd: HCI_CMD = 0x%04X, app_idx = %d, cmd_path = %d",
    ...
    APP_PRINT_TRACE3("app_anc_handle_anc_tool_cmd: HCI_CMD = 0x%02X %02X, result = %d",
                     buf[1], buf[0], buf[2]);

    app_report_event(cmd_path, EVENT_HCI, app_idx, buf, 3);
}
...
void app_anc_enter_measure_mode(uint8_t resp_meas_mode)
{
    ...
    anc_tool_trigger_wdg_reset((T_ANC_RESP_MEAS_MODE)resp_meas_mode);

    wdg_mode = resp_meas_mode ? RESET_ALL_EXCEPT_AON : RESET_ALL;
    app_wdg_reset(wdg_mode);
}

void app_anc_ini_wdg_reset(uint8_t mode)
{
    resp_meas_mode = mode;
    APP_PRINT_TRACE3("app_anc_ini_wdg_reset: %d, %d, link_num = %d", resp_meas_mode, mode,
    ...
    if (app_find_b2s_link_num())
    {
        ...
        gap_start_timer(&timer_handle_measure_kill_acl, "measure_kill_acl", anc_queue_id,
                        APP_IO_TIMER_ANC_MEASURE_KILL_ACL, 0, 3000);
    }
    else
    {
...
void app_anc_execute_wdg_reset(void)
{
    DBG_DIRECT("app_anc_execute_wdg_reset: mode = %d, anc_drv mode = %d", resp_meas_mode,
...
#else
    if ((app_hfp_get_call_status() != BT_HFP_CALL_IDLE) && (!app_cfg_const.enable_anc_when_sco))
#endif
    {
        return false;
    }
...
static void app_anc_timer_callback(uint8_t timer_id, uint16_t timer_chann)
{
    APP_PRINT_TRACE2("app_anc_timer_callback: timer_id 0x%02x, timer_chann %d",
                     timer_id, timer_chann);

    switch (timer_id)
    {
    case APP_IO_TIMER_ANC_MEASURE_KILL_ACL:
        {
            gap_stop_timer(&timer_handle_measure_kill_acl);
            ...
            for (idx = 0; idx < MAX_BR_LINK_NUM; idx++)
            {
                if (app_db.br_link[idx].connected_profile != 0)
                {
                    ...
                    {
                        legacy_send_acl_disconn_req(app_db.br_link[idx].bd_addr);
                    }
                }
            }
        }
    ...
    case APP_IO_TIMER_ANC_TOOL_WAIT_SEC_RESPOND:
        {
            gap_stop_timer(&timer_handle_anc_tool_wait_sec_respond);
    ...
    case BT_EVENT_ACL_CONN_SUCCESS:
        {
            if (anc_tool_check_resp_meas_mode() != ANC_RESP_MEAS_MODE_NONE)
            {
                if (remote_session_state_get() == REMOTE_SESSION_STATE_DISCONNECTED &&
                    (app_check_b2s_link(param->acl_conn_success.bd_addr)))
                {
    ...
    case BT_EVENT_ACL_CONN_FAIL:
        {
            ...
            {
                if (remote_session_state_get() == REMOTE_SESSION_STATE_DISCONNECTED &&
                    (app_check_b2s_link(param->acl_conn_fail.bd_addr)))
                {
...
static void app_anc_dm_cback(T_SYS_EVENT event_type, void *event_buf, uint16_t buf_len)
{
    ...
    if (handle == true)
    {
        APP_PRINT_INFO1("app_anc_dm_cback: event_type 0x%04x", event_type);
    }
}
                                ...
                                T_REMOTE_RELAY_STATUS status)
{
    ...
    case APP_REMOTE_MSG_RELAY_ANC_EVENT:
        {
            if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
            {
                ...
                {
                    memcpy(&anc_mp_tool_data.report_value[event_len], &p_info[4], event_len);
    ...
    case APP_REMOTE_MSG_SECONDARY_ANC_STATUS:
        {
            if (status == REMOTE_RELAY_STATUS_ASYNC_RCVD)
            {
                if (app_listening_report_secondary_state_condition())
                {
                    uint8_t remote_anc_status_on = *(uint8_t *)buf;

                    app_anc_report(EVENT_REPORT_STATUS, &remote_anc_status_on, 1);
                }
            }
        }
        break;
...
void app_anc_init(void)
{
    ...
    gap_reg_timer_cb(app_anc_timer_callback, &anc_queue_id);
