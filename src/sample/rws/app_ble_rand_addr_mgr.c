...
static void send_static_addr(uint8_t *random_addr)
{
...
void app_ble_rand_addr_handle_remote_conn_cmpl(void)
{
    ...
    if (memcmp(app_cfg_nv.le_rws_random_addr, (uint8_t[6]) {0}, 6) == 0)
    {
        ...
        go_through_cbs(&evt_data);
#if F_APP_ERWS_SUPPORT
        send_static_addr(app_cfg_nv.le_rws_random_addr);
#endif
    }
}
...
static void relay_parse(RELAY_MSG msg_type, uint8_t *buf, uint16_t len,
