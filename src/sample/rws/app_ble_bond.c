...
static void sync_mgr_default(void)
{
...
inline static void send_info(APP_LE_BOND_REMOTE_MSG  msg_id, APP_LE_BOND_REMOTE_INFO *p_info,
                             uint32_t len)
{
    if (len > BLE_BOND_MTU)
    {
...
inline static void batch_send_mgr_init(BATCH_SEND_MGR *p_send_mgr, uint32_t total_bond_num)
{
...
inline static void batch_send(BATCH_SEND_MGR *p_send_mgr, APP_LE_BOND_REMOTE_INFO *p_info,
...
#if F_APP_ERWS_SUPPORT
        send_info(APP_LE_BOND_MERGE, p_info, sizeof(*p_info) + dev_info_num * sizeof(T_LE_DEV_INFO));
#endif
    }
}
...
static void send_all_info(void)
{
    ...
    batch_send_mgr_init(&send_mgr, local_bond_num);
    ...
    if (p_info->total_bond_num == 0)
    {
        ...
        send_info(APP_LE_BOND_MERGE, p_info, sizeof(*p_info));
#endif
    ...
    batch_send(&send_mgr, p_info, bond_pri_table);
...
#if F_APP_ERWS_SUPPORT
        send_info(APP_LE_BOND_ADD, p_info, sizeof(*p_info) + sizeof(T_LE_DEV_INFO));
#endif
