...
tuya_ble_status_t tuya_ble_gap_disconnect(void)
{
    ...
    p_link = app_find_le_link_by_conn_id(le_tuya_adv_get_conn_id());

    if (p_link != NULL)
    {
        app_ble_disconnect(p_link, LE_LOCAL_DISC_CAUSE_TUYA);
        return TUYA_BLE_SUCCESS;
    }
    else
    {
...
tuya_ble_status_t tuya_ble_nv_init(void)
{
    ...
    if (p_tuya_nv_data)
    {
        uint32_t read_result = ftl_load_from_storage(p_tuya_nv_data, TUYA_NV_START_ADDR,
                                                     TUYA_NV_AREA_SIZE);
        if (read_result == FTL_READ_ERROR_READ_NOT_FOUND)
        {
            APP_PRINT_INFO0("tuya_ble_nv_init: initialize");
            memset(p_tuya_nv_data, 0, TUYA_NV_AREA_SIZE);
            ftl_save_to_storage(p_tuya_nv_data, TUYA_NV_START_ADDR, TUYA_NV_AREA_SIZE);
        }
...
tuya_ble_status_t tuya_ble_nv_write(uint32_t addr, const uint8_t *p_data, uint32_t size)
{
    ...
    if (ftl_save_to_storage((void *)p_data, addr, size) == 0)
    {
...
tuya_ble_status_t tuya_ble_nv_read(uint32_t addr, uint8_t *p_data, uint32_t size)
{
    ...
    if (ftl_load_from_storage((uint8_t *)p_data, addr, size) == 0)
    {
        return TUYA_BLE_SUCCESS;
    }
    else
