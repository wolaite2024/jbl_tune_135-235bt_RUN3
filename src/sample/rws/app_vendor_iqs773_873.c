...
void *timer_handle_iqs_release_debounce = NULL;
#if (APP_SENSOR_IQS773_873_VENDOR_FINE_TUNE_PARAM_LOG == 1)
void *timer_handle_iqs_addr_log = NULL;
#endif
void *timer_handle_iqs_sw_reset_check = NULL;

uint8_t psensor_iqs_timer_queue_id = 0;
static uint8_t iqs_force_set = 0;

/* App define PSENSOR_IQS timer type */
typedef enum
{
    APP_IO_TIMER_IQS_RELEASE_DEBOUNCE,
#if (APP_SENSOR_IQS773_873_VENDOR_FINE_TUNE_PARAM_LOG == 1)
    APP_IO_TIMER_IQS_ADDR_LOG,
#endif
    APP_IO_TIMER_IQS_773_SW_RESET_CHECK,
...
void i2c_iqs773_setup(void)
{
    ...
    // UI Tool Settings
    {
        ...
        i2c_iqs773_force_ati_tgt(app_cfg_const.iqs_p3.iqs773_sensitivity);
    ...
    // Debug String
    APP_PRINT_TRACE0("IQS773 Setup Finish1\r\n");
}
...
T_PSENSOR_I2C_EVENT i2c_iqs773_read_event(void)
{
    ...
    APP_PRINT_TRACE3("i2c_iqs773_read_event: %02x %02x %02x", iqs_event_flag, iqs_touch_flag,
                     iqs_data_buf[20]);
    ...
    if (((iqs_sys_flag & 0x80) == 0x80) || (iqs_data_buf[20] != 0xfa))
    {
        ...
        gap_start_timer(&timer_handle_iqs_sw_reset_check, "iqs773_sw_reset",
                        psensor_iqs_timer_queue_id,
                        APP_IO_TIMER_IQS_773_SW_RESET_CHECK, 0, 200);
    }
    else
    {
        // Prox UI Event
        if (iqs_event_flag & 0x01)
        {
            ...
            if ((iqs_touch_flag & 0x10) == 0x10)
            {
                ...
                if (timer_handle_iqs_release_debounce != NULL)
                {
                    //stop timer
                    gap_stop_timer(&timer_handle_iqs_release_debounce);
                ...
                if (iqs_force_set)
                {
                    if (app_cfg_const.iqs_release_debounce != 0)
                    {
                        ...
                        if (timer_handle_iqs_release_debounce == NULL)
                        {
                            ...
                            gap_start_timer(&timer_handle_iqs_release_debounce, "iqs_release_debounce",
                                            psensor_iqs_timer_queue_id,
                                            APP_IO_TIMER_IQS_RELEASE_DEBOUNCE, iqs_force_cnt, app_cfg_const.iqs_release_debounce * 10);
                        }
                    }
                    else
                    {
...
static void i2c_iqs_timer_callback(uint8_t timer_id, uint16_t timer_chann)
{
    switch (timer_id)
    {
    case APP_IO_TIMER_IQS_RELEASE_DEBOUNCE:
        {
            int16_t iqs_force_cnt = timer_chann;
...
#if (APP_SENSOR_IQS773_873_VENDOR_FINE_TUNE_PARAM_LOG == 1)
    case APP_IO_TIMER_IQS_ADDR_LOG:
        {
            gap_stop_timer(&timer_handle_iqs_addr_log);
            ...
            APP_PRINT_TRACE4("[idbg]i2c_iqs773_read_addr: addr_20:0x%02x, addr_21:0x%02x, addr_30:0x%02x addr_31:0x%02x",
                             iqs_data_dbg_20[0], iqs_data_dbg_20[1], iqs_data_dbg_30[0], iqs_data_dbg_30[1]);
            APP_PRINT_TRACE2("[idbg]i2c_iqs773_read_val: count: %d, LTA: %d",
    ...
    case APP_IO_TIMER_IQS_773_SW_RESET_CHECK:
        {
            gap_stop_timer(&timer_handle_iqs_sw_reset_check);
...
{
    gap_stop_timer(&timer_handle_iqs_addr_log);
    gap_start_timer(&timer_handle_iqs_addr_log, "iqs_addr_log",
                    psensor_iqs_timer_queue_id,
                    APP_IO_TIMER_IQS_ADDR_LOG, 0, 50);
}
#endif

void i2c_iqs_initial(void)
{
    gap_reg_timer_cb(i2c_iqs_timer_callback, &psensor_iqs_timer_queue_id);
