...
ISR_TEXT_SECTION void  app_sensor_jsa_int_gpio_intr_handler(void)
{
    uint8_t int_status;
    T_IO_MSG gpio_msg;
    ...
    int_status = gpio_read_input_level(app_cfg_const.sensor_detect_pinmux);
    ...
    {
        GPIO->INTPOLARITY &= ~(GPIO_GetPin(app_cfg_const.sensor_detect_pinmux)); //Polarity Low
    }
    else
    {
        uint8_t val = 0;

        GPIO->INTPOLARITY |= GPIO_GetPin(app_cfg_const.sensor_detect_pinmux); //Polarity High
        ...
        {
            APP_PRINT_ERROR0("sensor_int_gpio_intr_handler_detect: Send msg error");
        }
    }
