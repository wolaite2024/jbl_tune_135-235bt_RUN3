 ...
 *
 */
static uint8_t gfps_finder_advdata[31] =
{
    0x02,//length = 0x02
...
static uint8_t gfps_finder_scanrspdata[31] =
{
    12,//length = 0x02
...
T_BLE_EXT_ADV_MGR_STATE gfps_finder_get_adv_state()
{
...
void gfps_finder_adv_start(uint16_t duration_10ms)
{
    ...
    /*if device not in active UTP mode, update RPA when start adv or update adv EI*/
    if (app_gfps_finder_get_utp_mode() == false)
    {
        cause = gfps_finder_adv_update_rpa();
    }

    APP_PRINT_TRACE3("gfps_finder_adv_start: duration %d, adv_state %d, rpa update cause %d",
                     duration_10ms, adv_state, cause);
...
{
    APP_PRINT_TRACE2("gfps_finder_adv_stop: app_cause 0x%x, adv_state %d", app_cause, adv_state);
...
void gfps_finder_adv_update_frame_type(uint8_t frame_type)
{
    ...
    ble_ext_adv_mgr_set_adv_data(gfps_finder_adv_handle, sizeof(gfps_finder_advdata),
                                 gfps_finder_advdata);
}
 ...
 * @param hash
 */
void gfps_finder_update_adv_ei_hash(uint8_t *p_ei, uint8_t hash)
{
    if (p_ei)
    {
    ...
    ble_ext_adv_mgr_set_adv_data(gfps_finder_adv_handle, sizeof(gfps_finder_advdata),
                                 gfps_finder_advdata);
    APP_PRINT_TRACE2("gfps_finder_update_adv_ei_hash: EI %b, hash result 0x%x", TRACE_BINARY(20, p_ei),
                     hash);
}
    ...
    case BLE_EXT_ADV_SET_CONN_INFO:
        {
            ...
            APP_PRINT_TRACE1("gfps_finder_adv_callback: BLE_EXT_ADV_SET_CONN_INFO conn_id %d", conn_id);
            app_gfps_finder_beacon_set_conn_id(conn_id);
            app_reg_le_link_disc_cb(cb_data.p_ble_conn_info->conn_id, app_gfps_finder_beacon_le_disconnect_cb);
        }
        break;
    };
}
