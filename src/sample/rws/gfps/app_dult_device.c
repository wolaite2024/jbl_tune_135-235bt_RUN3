 ...
 */
static uint8_t           dult_timer_queue_id = 0;
static void              *timer_handle_dult_addr_update = NULL;
static void              *timer_handle_dult_exit_id_read_state = NULL;
static void              *timer_handle_dult_ring_duration = NULL;
...
{
    gap_stop_timer(&timer_handle_dult_addr_update);
    if (current_state == DULT_NEARING_OWNER)
    {
        gap_start_timer(&timer_handle_dult_addr_update, "update_addr",
                        dult_timer_queue_id,
                        APP_DULT_DEVICE_TIMER_UPDATE_ADDR, 0,
...
{
    keep_ring = true;
    gap_start_timer(&timer_handle_dult_ring_duration, "dult_ring_duration",
                    dult_timer_queue_id,
                    APP_DULT_DEVICE_TIMER_RING_DURATION, 0,
...
{
    keep_ring = false;
    gap_stop_timer(&timer_handle_dult_ring_duration);
    APP_PRINT_INFO0("app_dult_ring_duration_timer_stop");
}

static T_APP_AUDIO_TONE_TYPE app_dult_get_tone_type(void)
{
    T_APP_AUDIO_TONE_TYPE tone_type = app_cfg_nv.language_status == 0 ? TONE_ANC_SCENARIO_5 : TONE3_RSV;
    return tone_type;
}
...
void app_dult_device_sound_start(T_SERVER_ID service_id, T_DULT_CALLBACK_DATA *p_callback)
{
    if (app_audio_tone_type_play(app_dult_get_tone_type(), false, false))
    {
        ...
        app_dult_report_cmd_rsp(p_callback->conn_id, service_id,
    ...
    case AUDIO_EVENT_VOICE_PROMPT_STOPPED:
        {
            ...
            if (event_type == AUDIO_EVENT_VOICE_PROMPT_STOPPED)
            {
                ...
                {
                    tone_stopped_idx = VOICE_PROMPT_INDEX;
                }
                else
#endif
                {
            ...
            APP_PRINT_TRACE3("[sd_check]app_dult_audio_cback %d,%d,%d", event_type,
                             param->voice_prompt_stopped.index, tone_stopped_idx);

            if (tone_stopped_idx == app_cfg_const.tone_anc_scenario_5 || //dult tone or dult vp
                tone_stopped_idx == app_cfg_const.tone3_rsv
#if F_APP_HARMAN_FEATURE_SUPPORT
                ||  tone_stopped_idx == VOICE_PROMPT_INDEX
#endif
               )
...
void app_dult_id_read_state_enable(void)
{
    ...
    gap_start_timer(&timer_handle_dult_exit_id_read_state, "exit_id_read_state",
                    dult_timer_queue_id,
                    APP_DULT_DEVICE_TIMER_EXIT_ID_READ_STATE, 0,
...
void app_dult_device_init(void)
{
    ...
    gap_reg_timer_cb(app_dult_timer_cb, &dult_timer_queue_id);
}
