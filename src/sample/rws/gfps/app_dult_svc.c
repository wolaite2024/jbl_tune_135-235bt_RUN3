...
uint8_t GATT_UUID_NON_OWNER_SRV[16] = {0x85, 0x2A, 0x9F, 0x57, 0xC5, 0x2A, 0xED, 0x88, 0x26, 0xC2, 0xF4, 0x12, 0x01, 0x00, 0x19, 0x15};
...
static P_FUN_SERVER_GENERAL_CB  pfn_dult_cb = NULL;
...
T_APP_RESULT dult_attr_write_cb(uint8_t conn_id, T_SERVER_ID service_id,
...
void dult_cccd_update_cb(uint8_t conn_id, T_SERVER_ID service_id, uint16_t index,
                         uint16_t ccc_bits)
{
    bool handle = true;
...
*/
const T_FUN_GATT_SERVICE_CBS  dult_cbs =
{
    NULL,                       // read
...
*/
bool dult_send_indication(uint8_t conn_id, T_SERVER_ID service_id,
                          uint8_t *p_data, uint16_t data_len)
{
    APP_PRINT_INFO0("dult_send_indication");
    return server_send_data(conn_id, service_id,
...
*/
bool app_dult_report_cmd_rsp(uint8_t conn_id, T_SERVER_ID service_id,
                             uint16_t opcode, T_DULT_CMD_RSP_STATE state)
{
    ...
    return dult_send_indication(conn_id, service_id, (uint8_t *)&rpt, sizeof(rpt));
}

/**
 * @brief Add DULT service
*/
T_SERVER_ID dult_add_service(void *p_func)
{
    ...
    pfn_dult_cb = (P_FUN_SERVER_GENERAL_CB)p_func;
    return service_id;
}
#endif
