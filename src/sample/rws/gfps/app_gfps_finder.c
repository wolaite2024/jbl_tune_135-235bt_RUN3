...
typedef struct
{
    ...
    void *timer_idx_finder_update_ei;
    void *timer_idx_finder_update_random_window;
    void *timer_idx_finder_utp_enable_update_rpa;
...
void app_gfps_finder_set_default_value(void)
{
    ...
    p_app_gfps_finder->timer_idx_finder_update_ei = NULL;
    p_app_gfps_finder->timer_idx_finder_update_random_window = NULL;
    p_app_gfps_finder->timer_idx_finder_utp_enable_update_rpa = NULL;
...
void app_gfps_finder_start_update_adv_ei_timer()
{
    ...
    gap_start_timer(&p_app_gfps_finder->timer_idx_finder_update_ei, "gfps_finder_update_ei",
                    p_app_gfps_finder->gfps_finder_timer_id, GFPS_FINDER_UPDATE_ADV_EI, 0,
                    p_app_gfps_finder->gfps_finder_timeout_ms);
}
...
void app_gfps_finder_stop_update_adv_ei_timer()
{
    ...
    gap_stop_timer(&p_app_gfps_finder->timer_idx_finder_update_ei);
}
...
{
    APP_PRINT_TRACE0("app_gfps_finder_start_update_random_window_timer");
    gap_start_timer(&p_app_gfps_finder->timer_idx_finder_update_random_window,
                    "gfps_finder_update_random_window",
                    p_app_gfps_finder->gfps_finder_timer_id, GFPS_FINDER_UPDATE_RANDOM_WINDOW, 0,
                    platform_random(204) * 1000);
}
...
void app_gfps_finder_stop_update_random_window_timer()
{
    ...
    gap_stop_timer(&p_app_gfps_finder->timer_idx_finder_update_random_window);
}
...
void app_gfps_finder_upt_enable_start_update_rpa_timer()
{
    ...
    gap_start_timer(&p_app_gfps_finder->timer_idx_finder_utp_enable_update_rpa,
...
void app_gfps_finder_upt_enable_stop_rpa_timer()
{
    ...
    gap_stop_timer(&p_app_gfps_finder->timer_idx_finder_utp_enable_update_rpa);
}
...
void app_gfps_finder_timeout_cb(uint8_t timer_id, uint16_t timer_chann)
{
    ...
    case GFPS_FINDER_UPDATE_ADV_EI:
        {
            if (p_app_gfps_finder->timer_idx_finder_update_ei != NULL)
            {
    ...
    case GFPS_FINDER_UPDATE_RANDOM_WINDOW:
        {
            if (p_app_gfps_finder->timer_idx_finder_update_random_window != NULL)
            {
                ...
                gfps_finder_update_adv_ei_hash(p_app_gfps_finder->p_finder->adv_ei, hash);
    ...
    case GFPS_FINDER_UTP_ENABLE_UPDATE_RPA:
        {
            if (p_app_gfps_finder->timer_idx_finder_utp_enable_update_rpa != NULL)
            {
...
void app_gfps_finder_handle_power_off(void)
{
    ...
    if (p_app_gfps_finder->gfps_finder_beacon_conn_id != 0xFF)
    {
        ...
        if (p_link != NULL)
        {
            app_ble_disconnect(p_link, LE_LOCAL_DISC_CAUSE_GFPS_FINDER);
        }
    }
...
void app_gfps_finder_rtc_wakeup_save_info(void)
{
    ...
    ret_clock = ftl_save_to_storage(&p_app_gfps_finder->p_finder->clock_value,
                                    GFPS_FINDER_CLOCK_FLASH_OFFSET, 4);
    DBG_DIRECT("app_gfps_finder_rtc_wakeup_save_info: saved clock_value %d, status %d",
               p_app_gfps_finder->p_finder->clock_value, ret_clock);
}
...
void app_gfps_finder_enter_beacon_state(T_GFPS_FINDER_BEACON_STATE beacon_state)
{
    ...
    if (app_db.device_state != APP_DEVICE_STATE_ON &&
        extend_app_cfg_const.disable_finder_adv_when_power_off)
    {
        APP_PRINT_WARN0("app_gfps_finder_enter_beacon_state: will not broadcast finder adv when power off");
        return;
    }
...
bool app_gfps_finder_send_ring_rsp(uint8_t ring_state)
{
    ...
    if (app_cfg_nv.bud_role != REMOTE_SESSION_ROLE_SECONDARY)
    {
        ...
        uint16_t ring_time = app_gfps_rfc_get_ring_timeout();
...
void app_gfps_finder_send_eddystone_capability(uint8_t *bd_addr, uint8_t local_server_chann)
{
    ...
    ret = bt_gfps_send_eddystone_capability(bd_addr, local_server_chann,
    ...
    case GFPS_FINDER_EVT_RING:
        {
            ...
            app_gfps_rfc_set_ring_timeout(ring_time);
...
{
    p_app_gfps_finder = os_mem_zalloc(RAM_TYPE_DATA_ON, sizeof(T_APP_GFPS_FINDER));

    if (p_app_gfps_finder)
    {
        app_gfps_finder_set_default_value();
        p_app_gfps_finder->p_finder = os_mem_zalloc(RAM_TYPE_DATA_ON, sizeof(T_GFPS_FINDER));
        ...
        if (p_app_gfps_finder->p_finder)
        {
            ...
            gap_reg_timer_cb(app_gfps_finder_timeout_cb, &p_app_gfps_finder->gfps_finder_timer_id);
            return true;
        }
        else
        {
