...
static uint8_t     dult_conn_id = 0xFF;
static T_SERVER_ID dult_service_id = 0xFF;

void app_dult_handle_stack_ready(void)
{
...
void app_dult_sound_complete_handle(void)
{
    ...
    dult_send_indication(dult_conn_id, dult_service_id, (uint8_t *)&opcode, sizeof(opcode));
...
static T_DULT_BATT_LEVEL get_current_batt_level(void)
{
    ...
    APP_PRINT_INFO1("get_current_batt_level: %d", cur_level);
    if (cur_level < 10)
    {
...
{
    uint8_t conn_id = p_callback->conn_id;
    ...
    case DULT_OPCODE_INFO_GET_PRODUCT_DATA:
        {
            ...
            dult_send_indication(conn_id, service_id, (uint8_t *)&rpt, sizeof(rpt));
        }
        break;
    ...
    case DULT_OPCODE_INFO_GET_MODEL_NAME:
        {
            ...
            // uint8_t name[] = DULT_MODLE_NAME;
            // memcpy(rpt.model_name, name, sizeof(name));
    ...
    case DULT_OPCODE_CTRL_GET_IDENTIFIER:
        {
            ...
            {
                app_dult_report_cmd_rsp(conn_id, service_id,
...
{
    dult_service_id = dult_add_service(app_dult_cb);
}
