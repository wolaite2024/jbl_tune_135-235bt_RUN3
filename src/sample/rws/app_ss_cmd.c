...
static const T_SS_CLI_CMD ss_cli_cmd_list[] =
{
    ...
    /*Help*/
    {"Print Help menu",                   0xFFFF,   NULL,      "none",                             cmd_help                 },
...
void app_ss_handle_msg(T_IO_MSG *msg)
{
...
static void ss_send_msg(uint16_t cmd_id, void *param)
{
    T_IO_MSG msg;
...
static bool ss_common(const char *cmd_str, char *buf, size_t buf_len)
{
    uint16_t cmd_len;
...
static bool ss_all(const char *cmd_str, char *buf, size_t buf_len)
{
    cur_cmd_path = CMD_PATH_NONE;

    ss_common(cmd_str, buf, buf_len);

    return false;
}

static bool ss_exit(const char *cmd_str, char *buf, size_t buf_len)
{
...
static bool ss_help(const char *cmd_str, char *buf, size_t buf_len)
{
    return cli_help_set(&ss_cmd_help, cmd_str, buf, buf_len);
}

static bool ss_mode(const char *cmd_str, char *buf, size_t buf_len)
{
...
static bool match(const char *cmd_str, const char *p_cmd, size_t cmd_len)
{
    bool ret = false;
...
static const T_CLI_CMD ss_cmd_all =
{
    ...
    ss_all,           /* The function to run. */
    -1,               /* Zero or more parameter is expected. */
    match             /* 4 HEX command match pattern. */
};
    ...
    case CMD_SS_REQ:
        {
            ...
            is_exec = ss_common((const char *)&cmd_ptr[2], cmd_buf, cmd_buf_len);
            if (!is_exec)
            {
