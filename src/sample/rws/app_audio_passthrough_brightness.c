...
static uint8_t apt_brightness_queue_id = 0;
static uint32_t llapt_brightness_support_bitmap = 0;
static void *timer_handle_apt_brightness_cmd_wait_sec_respond = NULL;

typedef enum
{
    APP_IO_TIMER_APT_BRIGHTNESS_CMD_WAIT_SEC_RESPOND     = 0x00,
...
void app_apt_brightness_sync_handle(T_APT_BRIGHTNESS_RELAY_MSG *rev_msg)
{
    ...
    app_db.remote_main_brightness_level = rev_msg->main_brightness_level;
    app_db.remote_sub_brightness_level = rev_msg->sub_brightness_level;
}
...
static void app_apt_brightness_timer_callback(uint8_t timer_id, uint16_t timer_chann)
{
    APP_PRINT_TRACE2("app_apt_brightness_timer_callback: timer_id 0x%02x, timer_chann %d",
                     timer_id, timer_chann);

    switch (timer_id)
    {
    case APP_IO_TIMER_APT_BRIGHTNESS_CMD_WAIT_SEC_RESPOND:
        {
            if (timer_handle_apt_brightness_cmd_wait_sec_respond != NULL)
            {
                gap_stop_timer(&timer_handle_apt_brightness_cmd_wait_sec_respond);
    ...
    case CMD_LLAPT_BRIGHTNESS_SET:
        {
                ...
                (path != CMD_PATH_RWS_ASYNC))
            {
                ...
                {
                    gap_start_timer(&timer_handle_apt_brightness_cmd_wait_sec_respond, "apt_cmd_wait_sec_respond",
                                    apt_brightness_queue_id,
                                    APP_IO_TIMER_APT_BRIGHTNESS_CMD_WAIT_SEC_RESPOND, 0, 3000);
                }
                else
                {
...
void app_apt_brightness_init(void)
{
    ...
    gap_reg_timer_cb(app_apt_brightness_timer_callback, &apt_brightness_queue_id);
