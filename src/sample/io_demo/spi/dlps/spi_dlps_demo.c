...
void SPI_DemoCode(void)
{
    ...
    for (idx = 0; idx < len; idx++)
    {
        ...
        DBG_BUFFER(LOG_TYPE, SUBTYPE_FORMAT, MODULE_SPI, LEVEL_INFO,
...
void IOTestTask(void *pParameters)
{
    ...
    while (true)
    {
        if (xQueueReceive(DemoIOEventQueue, &event, portMAX_DELAY) == pdPASS)
        {
            ...
            {
                DBG_BUFFER(LOG_TYPE, SUBTYPE_FORMAT, MODULE_SPI, LEVEL_INFO, "IO test-->Task is running!",
                           0);
                /*-------print SPI register-----------------*/
                for (idx = 0; idx < 13; idx++)
                {
                    DBG_BUFFER(LOG_TYPE, SUBTYPE_FORMAT, MODULE_SPI, LEVEL_INFO, "SPI0_StoreReg[%d] = 0x%2x!", 2,
                               idx, *((uint32_t *)(0x40042000UL + idx * 4)));
                }
                SPI_DemoCode();
            }
...
{
    DBG_BUFFER(LOG_TYPE, SUBTYPE_FORMAT, MODULE_DLPS, LEVEL_INFO, "DlpsExitCallback", 0);
...
int main(void)
{
    ...
    xReturn = xTaskCreate(IOTestTask, "IO driver test", IOTest_STACK_SIZE / sizeof(portSTACK_TYPE),
                          NULL, IOTest_PRIORITY, NULL);
